#!/usr/bin/env node
/**
 * block-secret-leak.mjs — Kimi Secret Bridge Hook
 *
 * Two modes:
 ...
 *
 * Exit codes (Kimi hook convention):
 *   0 = allow
 *   2 = block
 *
 * stdin: JSON payload from Kimi Code hook system (snake_case fields)
 */
...
if (!mode || !["user-prompt", "pre-tool-use"].includes(mode)) {
  ...
  process.exit(0); // fail-open: don't block if invoked incorrectly
}
...
function handleUserPrompt() {
  ...
  if (!text || typeof text !== "string") {
    process.exit(0); // nothing to check — allow
  }

  const detected = detectToken(text);
  if (detected) {
    console.error(
      `[Kimi Secret Bridge] Blocked: detected ${detected} in your message.`
    );
    console.error(
      "Do not paste secrets into the conversation."
    );
    console.error(
      "Write them to .kimi-code/secret-bridge/secrets.local.env instead."
    );
    console.error(
...
function handlePreToolUse() {
  ...
  // For pure wrapper invocation (no sub-command to check), allow
  if (
    /\brun-with-secrets\.mjs\b/.test(command) &&
    !wrapperMatch
  ) {
    // Wrapper without -- (e.g. just checking args, or --check mode)
    process.exit(0);
  }

  // ── Helper: check all commands (full + wrapper sub-command) ──

  function blockWith(msg) {
    console.error(`[Kimi Secret Bridge] Blocked: ${msg}`);
    console.error(
      "Use the wrapper: node .kimi-code/secret-bridge/run-with-secrets.mjs -- <command>"
    );
    process.exit(2);
  }
  ...
  const ENV_DUMP_PATTERNS = [
    ...
    /^\s*env\s*[;|&]/, // env; ... or env | ... or env &&
    /^\s*printenv\s*[;|&]/, // printenv; ... or printenv | ...
    /\bprintenv\s+[A-Z_]+/, // printenv SPECIFIC_KEY
    /\becho\s+\$[A-Z_]*(KEY|TOKEN|SECRET|PASSWORD|CREDENTIAL|AUTH)/i, // echo $SECRET
    /process\.env\b/, // node -e "console.log(process.env)"
    /os\.environ/, // python -c "import os; print(os.environ)"
  ];
  ...
  for (const cmd of commandsToCheck) {
    let match;
    SENSITIVE_VAR_PATTERN.lastIndex = 0; // reset regex state
