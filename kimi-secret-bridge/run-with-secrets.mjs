#!/usr/bin/env node
/**
 * run-with-secrets.mjs — Kimi Secret Bridge Wrapper
 *
...
for (let i = 0; i < args.length; i++) {
  if (args[i] === "--require") {
    ...
    // Unknown flag before --, show usage
...
// --check mode: just validate keys, no child process
if (checkMode) {
  // Secrets will be read below, then validated
}
...
for (const rawLine of secretsContent.split(/\r?\n/)) {
  ...
  // Skip empty values (user hasn't filled in yet)
  if (!value) continue;
...
if (requiredKeys.length > 0) {
  ...
  if (missing.length > 0) {
    if (checkMode) {
      // --check mode: report status per key
    ...
    console.error("");
    console.error(
      "Add them to .kimi-code/secret-bridge/secrets.local.env"
    );
...
// Determine shell invocation for cross-platform compatibility
const isWindows = process.platform === "win32";
let spawnCommand, spawnArgs;

if (isWindows) {
  // On Windows, use cmd.exe to handle .cmd/.ps1 scripts (npm.cmd, pnpm.cmd)
  spawnCommand = "cmd";
  spawnArgs = ["/c", ...command];
} else {
